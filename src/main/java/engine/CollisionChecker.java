...
public class CollisionChecker {
	...
	public void checkTile(Entity e) {
	    ...
	    int rightCol = Math.min(loop.getHandler().getMapNum()[0].length - 1, rightX / GameVariables.TILE_SIZE);
	    int topRow = Math.max(0, topY / GameVariables.TILE_SIZE);
	    int bottomRow = Math.min(loop.getHandler().getMapNum().length - 1, bottomY / GameVariables.TILE_SIZE);
	    ...
	    switch(e.direction) {
	        case UP: 
	            ...
	            break;
	        case DOWN: 
	            bottomRow = Math.min(loop.getHandler().getMapNum().length - 1, (int) ((bottomY + e.speed) / GameVariables.TILE_SIZE));
